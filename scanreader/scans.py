...
class BaseScan:
    ...
    def num_frames(self):
        ...
        Each tiff page is an image at a given channel, at a different scanning depth. Slow stack acquisition collects all
        """
        if self.is_slow_stack:
            num_frames = min(
...
class ScanMultiROI(NewerScan, BaseScan):
    ...
    def __getitem__(self, key):
        # Fill key to size 5 (raises IndexError if more than 5)
        full_key = utils.fill_key(
            key, num_dimensions=5
        )  # key represents the scanfield index
